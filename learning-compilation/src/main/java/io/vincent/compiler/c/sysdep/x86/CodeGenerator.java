package io.vincent.compiler.c.sysdep.x86;

import io.vincent.compiler.c.asm.*;
import io.vincent.compiler.c.entity.*;
import io.vincent.compiler.c.ir.*;
import io.vincent.compiler.c.sysdep.CodeGeneratorOptions;
...
public class CodeGenerator implements io.vincent.compiler.c.sysdep.CodeGenerator,
    ...
    private void generateImmediate(AssemblyCode file, long size, Expression node) {
    ...
    public Void visit(Call node) {
        for (Expression arg : ListUtils.reverse(node.args())) {
            compile(arg);
            as.push(ax());
        }
    ...
    // #@@range/compileStmt{
    private void compileStmt(Statement statement) {
        if (options.isVerboseAsm()) {
            if (statement.location() != null) {
                as.comment(statement.location().numberedLine());
            }
        }
    ...
    // #@@range/compile{
    private void compile(Expression n) {
        if (options.isVerboseAsm()) {
    ...
    public Void visit(Binary node) {
        ...
        Type t = node.type();
        // #@@}
        if (node.right().isConstant() && !doesRequireRegisterOperand(operator)) {
            // #@@range/Bin_const{
            compile(node.left());
            compileBinaryOp(operator, ax(t), node.right().asmValue());
            // #@@}
        }
        else if (node.right().isConstant()) {
            compile(node.left());
            loadConstant(node.right(), cx());
            compileBinaryOp(operator, ax(t), cx(t));
        }
    ...
    private boolean doesRequireRegisterOperand(Operator operator) {
        switch (operator) {
        case S_DIV:
        case U_DIV:
        case S_MOD:
    ...
    private void compileBinaryOp(Operator operator, Register left, Operand right) {
     ...
     */
    // #@@range/loadConstant{
    private void loadConstant(Expression node, Register reg) {
