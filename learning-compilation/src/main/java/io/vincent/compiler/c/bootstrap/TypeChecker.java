...
import io.vincent.compiler.c.core.Visitor;
import io.vincent.compiler.c.entity.CBCParameter;
...
import io.vincent.compiler.c.type.IntegerType;
...
class TypeChecker extends Visitor {
    ...
    private void check(StatementNode node) {
        visitStmt(node);
    }

    private void check(ExpressionNode node) {
        visitExpr(node);
    }
    ...
    public Void visit(BlockNode node) {
        ...
        for (StatementNode n : node.stmts()) {
            check(n);
        }
        return null;
    }
    ...
    private boolean checkLHS(ExpressionNode lhs) {
        if (lhs.isParameter()) {
    ...
    public Void visit(CondExpressionNode node) {
        super.visit(node);
        checkCond(node.cond());
    ...
    private ExpressionNode integralPromotedExpr(ExpressionNode expr) {
    ...
    private void expectsComparableScalars(BinaryOpNode node) {
        ...
        if (node.left().type().isPointer()) {
            ExpressionNode right = forcePointerType(node.left(), node.right());
            node.setRight(right);
            node.setType(node.left().type());
            return;
        }
        if (node.right().type().isPointer()) {
            ExpressionNode left = forcePointerType(node.right(), node.left());
            node.setLeft(left);
            node.setType(node.right().type());
            return;
        }
    ...
    private ExpressionNode forcePointerType(ExpressionNode master, ExpressionNode slave) {
    ...
    public Void visit(FuncallNode node) {
        ...
        Iterator<ExpressionNode> args = node.args().iterator();
        List<ExpressionNode> newArgs = new ArrayList<ExpressionNode>();
        // mandatory args
        for (Type param : type.paramTypes()) {
            ExpressionNode arg = args.next();
    ...
    private ExpressionNode castOptionalArg(ExpressionNode arg) {
    ...
    private boolean checkRHS(ExpressionNode rhs) {
    ...
    // Applied To: return expression, assignment RHS, funcall argument
    private ExpressionNode implicitCast(Type targetType, ExpressionNode expr) {
    ...
    private boolean mustBeInteger(ExpressionNode expr, String op) {
    ...
    private boolean mustBeScalar(ExpressionNode expr, String op) {
    ...
    private void wrongTypeError(ExpressionNode expr, String op) {
