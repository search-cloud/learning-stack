...
import io.vincent.compiler.c.exception.OptionParseError;
import io.vincent.compiler.c.parser.LibraryLoader;
import io.vincent.compiler.c.sysdep.*;
...
public class Options {
	...
	public static final String ProgramName = "vincent-compiler";
	...
	private AssemblerOptions assemblerOptions = new AssemblerOptions();
	...
	public CodeGenerator codeGenerator(ErrorHandler h) {
	...
	public AssemblerOptions assembilerOptions() {
		...
		while (args.hasNext()) {
			...
			if (arg.equals("--")) {
				...
				if (CompilerMode.isModeOption(arg)) {
					...
					loader.addLoadPath(getOptionArg(arg, args));
				} else if (arg.equals("--debug-parser")) {
					debugParser = true;
				} else if (arg.startsWith("-o")) {
					outputFileName = getOptionArg(arg, args);
					...
					for (String a : parseCommaSeparatedOptions(arg)) {
						assemblerOptions.addArg(a);
					}
				} else if (arg.equals("-Xassembler")) {
					assemblerOptions.addArg(nextArg(arg, args));
					...
					addLdArg("-L" + getOptionArg(arg, args));
				} else if (arg.startsWith("-l")) {
					addLdArg("-l" + getOptionArg(arg, args));
	...
	private String getOptionArg(String opt, ListIterator<String> args) {
	...
	private void printUsage(PrintStream out) {
		...
		// --dump-expression is a hidden option.
