...
import io.vincent.compiler.entity.DefinedVariable;
...
abstract public class Visitor implements ASTVisitor<Void, Void> {
    ...
    protected void visitStmt(StatementNode stmt) {
        stmt.accept(this);
    }

    protected void visitStmts(List<? extends StatementNode> stmts) {
        for (StatementNode s : stmts) {
            visitStmt(s);
        }
    }

    protected void visitExpr(ExpressionNode expr) {
        expr.accept(this);
    }

    protected void visitExprs(List<? extends ExpressionNode> exprs) {
    ...
    public Void visit(CondExpressionNode n) {
        visitExpr(n.cond());
