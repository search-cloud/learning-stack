package io.vincent.compiler.core;
...
public class IRGenerator implements ASTVisitor<Void, Expression> {
	...
	private io.vincent.compiler.asm.Type asmType(Type t) {
		if (t.isVoid()) return int_t();
		return io.vincent.compiler.asm.Type.get(t.size());
	}

	private io.vincent.compiler.asm.Type varType(Type t) {
		if (!t.isScalar()) {
			return null;
		}
		return io.vincent.compiler.asm.Type.get(t.size());
	...
	private io.vincent.compiler.asm.Type int_t() {
		return io.vincent.compiler.asm.Type.get(typeTable.intSize());
	}

	private io.vincent.compiler.asm.Type size_t() {
		return io.vincent.compiler.asm.Type.get(typeTable.longSize());
	}

	private io.vincent.compiler.asm.Type ptr_t() {
		return io.vincent.compiler.asm.Type.get(typeTable.pointerSize());
	}

	private io.vincent.compiler.asm.Type ptrdiff_t() {
		return io.vincent.compiler.asm.Type.get(typeTable.longSize());
	}
